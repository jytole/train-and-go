...
void loop() {
  ...
  gravArr[gravCount] = sqrt((accel.acceleration.x*accel.acceleration.x) + (accel.acceleration.y*accel.acceleration.y) + (accel.acceleration.z*accel.acceleration.z));
  ...
  for(int i = 0; i < 20; i++)
  {
    gravSum = gravSum+gravArr[i];
  }

  Serial.print("\t\tRolling average gravity (20 samples): ");
  Serial.print(gravSum/20);
  Serial.println();
